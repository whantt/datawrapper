...
<script>
    ...
    export function toggle() {
        ...
        if (id) {
            ...
            if (!visGroups[chart.type]) visGroups[chart.type] = {};
            visGroups[chart.type][id] = visGroups[chart.type][id] === 'open' ? 'closed' : 'open';
    ...
    onMount(() => {
        ...
        if (id) {
            const visGroups = JSON.parse(window.localStorage.getItem('dw-vis-groups')) || {};
            if (visGroups[chart.type] && visGroups[chart.type][id]) {
                open = visGroups[chart.type][id] !== 'closed';
            }
        }
    })
